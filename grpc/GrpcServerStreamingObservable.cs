...
public sealed class GrpcServerStreamingObservable<TResponse> : IObservable<TResponse>, IDisposable {
	...
	public async Task ObserveAsync(CancellationToken cancellation = default(CancellationToken)){
		try{
			while(!cancellation.IsCancellationRequested && await m_ResponseStream.MoveNext().ConfigureAwait(false)){
				OnNext(m_ResponseStream.Current);
			}
			Debug.Log("gsso before finally");
			OnCompleted();
		}catch(Exception ex){
			OnError(ex);
			throw;
	...
	private void OnCompleted(){
		Debug.Log("gsso oncompleted count: "+m_Observers.Count);
